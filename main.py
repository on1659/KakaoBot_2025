...
kakao_opentalk_name = '하트시그널 토론회장'
chat_command_Map = [
    ['#유툽', youtube.GetData],
    ['[카카오맵]', convert_naver_map.GetData],
    ['#every', every_mention.GetData]
]
...
# # 채팅내용 가져오기
def copy_chatroom(chatroom_name, hwndMain = 0):
    # # 핸들 _ 채팅방
...
# # 채팅방 열기
def open_chatroom(chatroom_name, focus_window=0):
...
# # 채팅내용 초기 저장 _ 마지막 채팅
def chat_last_save():
    open_chatroom(kakao_opentalk_name, 1)  # 채팅방 열기
    ttext = copy_chatroom(kakao_opentalk_name, 1)  # 채팅내용 가져오기
...
def chat_chek_command(cls, clst):
    ...
    copy_text = copy_chatroom(kakao_opentalk_name)  # 채팅내용 가져오기
    ...
    if df.iloc  [-2, 0] == clst:
        ...
        for chat_command, chat_func in chat_command_Map:
            ...
            if 1 <= int(found.count()):
                print("-------커멘드 확인!")
                resultString = chat_func(kakao_opentalk_name, chat_command, found.iloc[0])
                kakao_sendtext(kakao_opentalk_name, resultString)  # 메시지 전송
